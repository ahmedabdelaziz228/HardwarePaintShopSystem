...
import 'package:hardware_paint_shop_mobile/models.dart';
import 'package:provider/provider.dart';

/// Hosts the authenticated application's primary navigation and current RC1 features.
  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          Text('أهلًا، ${auth.userName}', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w800)),
...
class _ProductsPageState extends State<ProductsPage> {
  ...
  void initState() {
    ...
    Future.microtask(() => state.searchProducts(''));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: auth.can('Product.Create')
...
class _ProductDetailsPageState extends State<ProductDetailsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: auth.can('Inventory.Adjust')
...
class _CustomersPageState extends State<CustomersPage> {
  ...
  void initState() {
    super.initState();
    final state = context.read<AppState>();
    Future.microtask(() => state.searchCustomers(''));
  }
...
class _CustomerDetailsPageState extends State<CustomerDetailsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: auth.can('Finance.CustomerCollection')
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      InfoCard(children: [
        ...
        infoRow('آخر مزامنة', state.lastSync == null ? 'لم تتم' : intl.DateFormat('dd/MM/yyyy HH:mm').format(state.lastSync!.toLocal())),
      ...
      const SizedBox(height: 16),
      Card(child: ListTile(leading: const Icon(Icons.logout, color: Color(0xffdc2626)), title: const Text('تسجيل الخروج'), onTap: auth.logout)),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(16), children: [
        ...
        const SizedBox(height: 10),
        DropdownButtonFormField<String>(initialValue: unitId, decoration: const InputDecoration(labelText: 'الوحدة الأساسية *'), items: units.map<DropdownMenuItem<String>>((u) => DropdownMenuItem(value: '${u['id']}', child: Text('${u['name']}'))).toList(), onChanged: (v) => setState(() => unitId = v)),
        const SizedBox(height: 10),
        DropdownButtonFormField<String>(initialValue: categoryId, decoration: const InputDecoration(labelText: 'التصنيف'), items: categories.map<DropdownMenuItem<String>>((c) => DropdownMenuItem(value: '${c['id']}', child: Text('${c['name']}'))).toList(), onChanged: (v) => setState(() => categoryId = v)),
        const SizedBox(height: 10),
        DropdownButtonFormField<String>(initialValue: priceGroupId, decoration: const InputDecoration(labelText: 'فئة السعر'), items: groups.map<DropdownMenuItem<String>>((g) => DropdownMenuItem(value: '${g['id']}', child: Text('${g['name']}'))).toList(), onChanged: (v) => setState(() => priceGroupId = v)),
        const SizedBox(height: 10),
...
Future<void> showCollection(BuildContext context, CustomerSummary customer, {required VoidCallback onSaved}) async {
  ...
  await showDialog(context: context, builder: (dialogContext) => StatefulBuilder(builder: (context, setDialogState) => AlertDialog(
    ...
    content: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(height: 10),
      DropdownButtonFormField<String>(initialValue: cashboxId, decoration: const InputDecoration(labelText: 'الخزينة'), items: cashboxes.map<DropdownMenuItem<String>>((c) => DropdownMenuItem(value: '${c['id']}', child: Text('${c['name']}'))).toList(), onChanged: (v) => setDialogState(() => cashboxId = v)),
...
String money(dynamic value) => intl.NumberFormat.currency(locale: 'ar_EG', symbol: 'ج.م', decimalDigits: 2).format(asDouble(value));
