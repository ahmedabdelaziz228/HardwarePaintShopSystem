...
- طباعة فاتورة بيع تفصيلية A4 أو حرارية، وحفظها PDF، وطباعة كشوف حساب العملاء، وتصدير CSV.
...
## 13. البيع بوحدات مختلفة والفواتير الآجلة

### الوحدات والتحويلات

كل منتج له وحدة أساسية واحدة، وكل الوحدات الأخرى تتحول مباشرة إليها:

- الفحم: الوحدة الأساسية `كيلوجرام`، والشكارة معاملها `25`. شراء 25 شكارة يضيف 625 كجم، ويمكن البيع بالشكارة أو بالكيلو من نفس الرصيد.
- السلك: الوحدة الأساسية `متر`، واللفة معاملها `50`. شراء لفة يضيف 50 متر، وكل بيع بالمتر يخصم المقدار الفعلي ويُظهر المتبقي بدقة.
- الكميات تدعم الكسور حتى ثلاث منازل عشرية، ولذلك يمكن بيع 0.250 كجم أو 12.5 متر.

سعر البيع المقترح وأقل سعر يُسجلان لكل وحدة وفئة سعر. الكاشير يستطيع كتابة السعر الفعلي، والخدمة تمنع النزول عن الحد الأدنى إلا بصلاحية خاصة. متوسط تكلفة الشراء يُلتقط داخل كل سطر عند ترحيل البيع حتى تظل تقارير الربح تاريخية وصحيحة بعد تغيّر سعر الشراء، كما تعكس تكلفة مرتجعات البيع.

### الفاتورة والعميل الآجل

- البيع النقدي المدفوع بالكامل لا يحتاج إنشاء عميل ويظل محفوظًا في سجل المبيعات.
- عند وجود باقي دون اختيار عميل، تظهر بيانات العميل الآجل ويُنشأ العميل تلقائيًا داخل نفس معاملة الترحيل.
- الفاتورة تعرض الأصناف والوحدات والكميات وسعر الوحدة والإجماليات والخصم والمدفوع والباقي ورصيد العميل قبل وبعد الفاتورة.
- من شاشة العملاء يمكن فتح كشف حساب مؤرخ وطباعته أو حفظه PDF.
- بيانات المنشأة والعنوان والهاتف والرقم الضريبي والسجل التجاري والشعار وعنوان الفاتورة والتذييل والعملة ومقاس الورق تُعدل من شاشة الإعدادات.
- للحفظ PDF اختر `Microsoft Print to PDF` من نافذة الطباعة.

## 14. الملفات المهمة للمطور
